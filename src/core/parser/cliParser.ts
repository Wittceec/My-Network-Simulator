...
interface ParserResult { output: string[]; newMode: CliMode; newContext: string; }

export function executeCommand(rawInput: string, currentMode: CliMode, currentContext: string, device: Device): ParserResult {
  const input = rawInput.trim(); const args = input.split(/\s+/); const cmd = args[0].toLowerCase();
  ...
  if (cmd === 'exit') {
    if (currentMode === 'interface' || currentMode === 'dhcp') { result.newMode = 'global'; result.newContext = ''; } 
    else if (currentMode === 'global') { result.newMode = 'privilege'; } 
    else if (currentMode === 'privilege') { result.newMode = 'user'; }
    return result;
  } 
  if (cmd === 'end') {
    if (currentMode === 'global' || currentMode === 'interface' || currentMode === 'dhcp') { result.newMode = 'privilege'; result.newContext = ''; return result; }
  }
    ...
    case 'privilege':
      if (cmd === 'configure' || cmd === 'conf') {
        ...
        if (!targetIp) result.output.push('% Incomplete command.');
          ...
          if (pingSuccess) { result.output.push('!!!!!\nSuccess rate is 100 percent (5/5)'); } 
    ...
    case 'global':
      if (cmd === 'hostname') {
        ...
        if (network && mask && nextHop) {
          updateDevice(device.id, (d) => ({ ...d, routingTable: [...d.routingTable, { network, mask, nextHopIp: nextHop, protocol: 'static', metric: 1 }] }));
        }
        ...
        result.newMode = 'interface'; result.newContext = args[1];
      } else if (cmd === 'vlan') {
