...
function generateSysAdminTicket(): Ticket | null {
  ...
  if (rand < 0.1) {
    ...
    return createTicket('Block Executables on Public Share', 'Add ".exe" to the File Screening block list on the "Public" share.', 'SysAdmin', 'High', 'fs_enable_screening');
  } else {
    return createTicket('Block Inheritance on Sales OU', 'Sales wants to manage their own policies. Block inheritance on the "Sales" OU.', 'SysAdmin', 'Medium', 'ad_block_inheritance');
  }
}
...
function generateNetAdminTicket(): Ticket | null {
  ...
  if (devices.length > 0 && rand < 0.5) {
    const router = devices.find(d => d.type === 'router');
    if (router) return createTicket(`OSPF neighbor down on ${router.name}`, `Investigate OSPF neighbor relationship on ${router.name}.`, 'NetAdmin', 'Critical', 'net_ospf_down', router.id);
  } else {
...
export function forceGenerateTicket() {
  ...
  if (generator) {
    ...
    // Try a few times if null
    let attempts = 0;
    while (!newTicket && attempts < 5) {
      newTicket = generator();
      attempts++;
    }
    if (newTicket) jobStore.addTicket(newTicket);
  }
}
